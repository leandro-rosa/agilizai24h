...
import { ALL_PARAMETER_PATHS, lossBusinessRuleRows, lossParameterCatalogSections } from "./parameter-rows";
...
describe("lossParameterCatalogSections", () => {
  const sections = lossParameterCatalogSections(DEFAULT_PARAMETERS);
  ...
  it("matches formatParameterValue and envNameOf from parameters.ts for a known path", () => {
    const path: ParameterPath = "priority.criticalValueCents";
  ...
  it("marks isOverridden when the value differs from the given defaults", () => {
    const overridden = { ...DEFAULT_PARAMETERS, priority: { ...DEFAULT_PARAMETERS.priority, criticalValueCents: 50000 } };
    const overriddenSections = lossParameterCatalogSections(overridden, DEFAULT_PARAMETERS);
    const row = overriddenSections.flatMap((section) => section.rows).find((candidate) => candidate.path === "priority.criticalValueCents");
    expect(row?.isOverridden).toBe(true);
    expect(row?.formattedValue).toBe(formatParameterValue("priority.criticalValueCents", 50000));
    expect(row?.fallbackFormattedValue).toBe(formatParameterValue("priority.criticalValueCents", DEFAULT_PARAMETERS.priority.criticalValueCents));
  });
});

describe("lossBusinessRuleRows", () => {
  const businessRows = lossBusinessRuleRows(DEFAULT_PARAMETERS);
