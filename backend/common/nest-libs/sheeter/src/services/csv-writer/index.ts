...
import { SearchCriteriaInterface } from '@app/prisma-db-client/criteria'
...
export class CsvWriterService {
  ...
  async appendRowData({ data, filePath }: { filePath: string; data: Record<string, any> }): Promise<string> {
    try {
      ...
      throw new Error(`Erro ao adicionar o registro: ${(error as Error).message}`)
    }
  }
  ...
  async appendRowsData({
    ...
    try {
      ...
      throw new Error(`Erro ao adicionar os registros: ${(error as Error).message}`)
    }
  }
  ...
  async uploadToS3({ filePath, s3Key }: { filePath: string; s3Key: string }): Promise<string> {
    try {
      ...
      throw new Error(`Erro ao enviar o arquivo para o S3: ${(error as Error).message}`)
    }
  }
}
