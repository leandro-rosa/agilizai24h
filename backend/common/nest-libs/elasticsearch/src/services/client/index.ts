...
export class ElasticsearchClientService implements OnModuleInit {
  ...
  /**
   * Plain ad hoc search — for point-lookup use cases (manual product search,
   * per-item matching) where PIT pagination (fetchPage/openPIT/closePIT,
   * built for bulk scans) is the wrong tool. First consumer: apps/quote's
   * matching worker and manual-search endpoint.
   */
  public async search<T>(index: string, query: Record<string, unknown>, size = 10): Promise<T[]> {
    if (!this.INSTANCE) {
      throw new Error('Elasticsearch client instance is not initialized')
    }

    const response = await this.INSTANCE.search<T>({ index, query, size })
    return (response?.hits?.hits || []).map((hit: any) => hit._source as T)
  }

  public async mget<T>(index: string, ids: string[]): Promise<T[]> {
    if (!this.INSTANCE) {
      throw new Error('Elasticsearch client instance is not initialized')
    }
    if (!ids.length) {
    ...
    const response = await this.INSTANCE.mget<T>({ index, ids })
    return (response?.docs || [])
      .filter((doc: any) => doc.found)
      .map((doc: any) => doc._source as T)
  }
