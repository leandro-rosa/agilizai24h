...
import { Injectable, Inject, ConflictException, OnApplicationShutdown } from '@nestjs/common'
 ...
 */
@Injectable()
export class HoldItBullMQBroker implements HoldItBrokerInterface, OnApplicationShutdown {
    ...
    for (const queue of queues) {
      try {
        ...
        if (queueClient.status !== 'ready') {
          throw new Error(`Redis client status is "${queueClient.status}", expected "ready"`)
        }
      } catch (err) {
        details[queue.name] =
