...
export class BullMQService {
   ...
   *
   * BullMQ's IRedisClient abstraction only exposes `set(key, value, { PX, EX })`
   * (no NX), so the atomic NX+PX combination is issued as a tiny Lua script
   * via runCommand instead of a raw ioredis call.
   */
  private async acquireQueueLock(queue: Queue, key: string, ttlMs: number): Promise<boolean> {
    const client = await queue.client
    const value = `${process.pid}:${Date.now()}`
    client.defineCommand('holdItAcquireLock', {
      numberOfKeys: 1,
      lua: "return redis.call('SET', KEYS[1], ARGV[1], 'PX', ARGV[2], 'NX')",
    })
    const result = await client.runCommand('holdItAcquireLock', [key, value, String(ttlMs)])
    return result === 'OK'
  }
