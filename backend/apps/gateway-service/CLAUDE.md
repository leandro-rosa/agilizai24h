...
| `/treasury/accounts`, `/treasury/transactions`, `/treasury/mappings`, `/treasury/fees`, `/treasury/settlements`, `/treasury/categories` | `treasury:read` / `:write` | 23 rotas; `GET /treasury/transactions/summary` traz `unresolved_count`, `movement_cents`, `pending_count`/`pending_cents`; `.../by-supplier` consolida entre contas; `.../neutralization-candidates` só sugere, `POST .../neutralize` (`:write`) vincula; `PATCH .../transactions/bulk` (`:write`, declarada antes de `:id` de propósito) aplica `nature`/`category` em lote — `nature` só grava nas linhas selecionadas com `kind: expense`, ignora as demais sem erro |
